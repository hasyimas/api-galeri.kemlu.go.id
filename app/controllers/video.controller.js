...
exports.getAll = async (req, res) => {
    try {
        const galleries = await Galleries.find({ fileType: "video" })
...
exports.create = async (req, res) => {
    ...
    try {
        ...
        Galleries.insertMany(galleryArray).then((result) => {
            res.status(200).send({
                message: "Video has been inserted"
            });
        }).catch((err) => {
            res.status(400).send({
                message: err.message || "Some error while create Video"
            })
        });
